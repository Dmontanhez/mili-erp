...
IUGU_WEBHOOK_SECRET = "test_secret_iugu"
...
async def fetch_and_store_meli_order(order_id: str, seller_id: str | None):
    ...
    status = "paid"
    payload_text = '{"message": "Pedido simulado para testes locais"}'

    try:
        # Tenta buscar na API real do Mercado Livre caso exista token ativo
        token = await get_meli_token(seller_id)
        ...
        # Poka-Yoke: Se falhar (falta de token ou ambiente local sem conexão real),
        # utiliza o fallback simulado para que o teste de expedição funcione perfeitamente.
        pass
    ...
    cur.execute("""
        ...
        ON CONFLICT(order_id) DO UPDATE SET
            ...
            updated_at=excluded.updated_at
    """, (order_id, seller_id, status, payload_text, now_utc(), now_utc()))
    conn.commit()
    conn.close()
...
# --------------------------
# Rotas do App do Cliente (Estoque, Vendas & Autenticação)
# --------------------------
...
async def update_product_stock(sku: str, payload: StockUpdateSchema):
    ...
    return {"status": "success", "message": "Estoque local updated com sucesso!"}
...
def issue_nfe(order_id: str):
    ...
    return {
        "status": "success",
        "message": f"NF-e emitida com sucesso para o pedido {order_id}! As 2 etiquetas obrigatórias (Etiqueta de Envio + Etiqueta de Identificação/NF-e Simplificada) foram geradas em conjunto para a embalagem."
    }
